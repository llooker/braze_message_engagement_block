...
view: email_messaging_frequency {
  derived_table: {
    sql: SELECT date_trunc({% parameter date_granularity %}, to_timestamp(deliveries.time)) AS delivered_time,
        ...
        FROM prod_analytics.analytics_staging.stg_braze_email_deliveries  AS deliveries
        LEFT JOIN prod_analytics.analytics_staging.stg_braze_email_opens  AS opens ON (deliveries.email_address)=(opens.email_address)
                    AND
                    ((deliveries.message_variation_id)=(opens.message_variation_id)
                    OR
                    (deliveries.canvas_step_id)=(opens.canvas_step_id))
        LEFT JOIN prod_analytics.analytics_staging.stg_braze_email_clicks  AS clicks ON (deliveries.email_address)=(clicks.email_address)
                    AND
